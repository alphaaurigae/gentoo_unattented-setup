...
#######################################################################################################################
	...
	PREPARE_CHROOT () { # 3.0 PREPARE CHROOT
		...
		DL_STAGE () { # https://wiki.gentoo.org/wiki/Handbook:AMD64/Installation/Stage#Choosing_a_stage_tarball
			...
			tar xvJpf $CHROOTX/stage3.tar.xz --xattrs-include='*.*' --numeric-owner -C $CHROOTX # https://wiki.gentoo.org/wiki/Handbook:AMD64/Installation/Stage#Unpacking_the_stage_tarball
		...
		EBUILD () { # https://wiki.gentoo.org/wiki/Handbook:AMD64/Installation/Base#Gentoo_ebuild_repository
			...
			cp $CHROOTX/usr/share/portage/config/repos.conf /mnt/gentoo/etc/portage/repos.conf/gentoo.conf # copy the Gentoo repository configuration file provided by Portage to the (newly created) repos.conf directory.
...
PRESET_INPUDEVICE="libinput keyboard"
...
PRESET_USEFLAG='X a52 aac aalib acl acpi adns alsa atm audit bash-completion berkdb bidi blas boost branding bzip2 \
			...
			hardened highlight gzip ipv6 int64 introspection idn jack jpeg jemalloc kms lame latex ldap libcaca lm_sensors \
			lua lzma lzo lz4 m17n-lib matroska memcached mhash modules mount numa mp3 mp4 mpeg mtp nls ocaml opengl opus osc oss \
...
PRESET_FEATURES="sandbox binpkg-docompress binpkg-dostrip binpkg-dostrip candy cgroup clean-logs collision-protect \
				...
				network-sandbox noman parallel-fetch parallel-install pid-sandbox userpriv usersandbox webrsync-gpg"
...
PRESET_SYNC="rsync://rsync5.us.gentoo.org/gentoo-portage/ rsync://rsync25.us.gentoo.org/gentoo-portage/ \
			rsync://rsync3.ca.gentoo.org/gentoo-portage/ rsync://rsync1.au.gentoo.org/gentoo-portage/ \
			rsync://rsync3.jp.gentoo.org/gentoo-portage/"
...
BASE_SYSTEM () {
	...
	#                                                                                                                     
	CHOOSE_PROFILE () { # https://wiki.gentoo.org/wiki/Profile_(Portage)
	...
	MAKECONF () { # https://wiki.gentoo.org/wiki//etc/portage/make.conf
		...
		MAKECONF_VARIABLES () {
			...
			CPU_FLAGS_X86="${COMMON_CPUFLAGS}"
			CFLAGS="${COMMON_FLAGS}" # clone
			CXXFLAGS="${COMMON_FLAGS}" # clone https://wiki.gentoo.org/wiki/Handbook:AMD64/Installation/Stage#CFLAGS_and_CXXFLAGS
			FCFLAGS="${COMMON_FLAGS}"
			FFLAGS="${COMMON_FLAGS}"
			MAKEOPTS="$PRESET_MAKE"
			INPUT_DEVICES="$PRESET_INPUDEVICE"
...
EOF
	...
	CHOOSE_PROFILE		&& echo "${bold}CHOOSE_PROFILE - END ....${normal}"
				...
				INSTALL_LXDM () {
					...
					AUTOSTART_LXDM () {
						...
						LXDM_SYSTEMD () { # systemd switch (option variables top) 
							arch-chroot $CHROOTX systemctl enable lxdm
						}
						LXDM_$SYSINITVAR
					}
					EMERGE_LXDM
		...
		DESKTOP_ENV () {
			...
			XFCE4 () { # https://wiki.gentoo.org/wiki/Xfce
				...
				W_DISPLAYMGR () { # https://wiki.gentoo.org/wiki/Xfce#Display_managers
					XFCE4_LXDM () {
						sed -i -e 's;^# session=/usr/bin/startlxde;session=/usr/bin/startxfce4;g' $CHROOTX/etc/lxdm/lxdm.conf
					}
					XFCE4_LXDM
				}
